...
fn summary_by_issue_has_total_row() {
    ...
    assert_eq!(cell_float(&mut wb, "Summary by Issue", 3, 4), 5.0);
}
...
fn summary_by_issue_aggregates_hours() {
    ...
    assert_eq!(cell_float(&mut wb, "Summary by Issue", 1, 4), 3.0);
    assert_eq!(cell_str(&mut wb, "Summary by Issue", 2, 0), "PROJ-2");
    assert_eq!(cell_float(&mut wb, "Summary by Issue", 2, 4), 4.0);
}
